  ...
  <div>
    ...
    <ContentCard v-if="!showForm">
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex items-center gap-2 ml-4">
          <ActionButton icon="add" title="Tambah Galery" size="md" variant="filled" @click="handleAdd" />
          <ActionButton icon="delete" title="Hapus Galery" size="md" variant="filled" :disabled="!selectedItem" @click="handleDelete" />
          <ActionButton icon="view" title="Lihat Detail" size="md" variant="filled" :disabled="!selectedItem" @click="handleView" />
          <ActionButton icon="edit" title="Edit Galery" size="md" variant="filled" :disabled="!selectedItem" @click="handleEdit" />
          <ActionButton icon="copy" title="Copy Galery" size="md" variant="filled" :disabled="!selectedItem" @click="handleCopy" />
        </div>
      </div>
    ...
    <ContentCard v-else>
      ...
      <FormHeader :title="formConfig.title" :subtitle="formConfig.subtitle" @back="handleCancel">
        <template #actions>
          <ActionButton v-if="isViewMode" icon="edit" title="Edit Galery" size="md" variant="filled" @click="handleEdit" />
        </template>
      </FormHeader>
      ...
      <form @submit.prevent="handleFormSubmit">
        <div class="grid grid-cols-2 gap-x-8">
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center gap-4">
              ...
              <FormField v-model="formData.judul" type="text" placeholder="Judul Galery" :disabled="formConfig.readonly" class="flex-1" hide-label />
            </div>
        ...
        <div v-else class="flex justify-end gap-3 pt-6 border-t border-gray-200">
          ...
          <BaseButton type="button" variant="filled" color="blue" @click="handleEdit"> Edit Galery </BaseButton>
        </div>
      </form>
    </ContentCard>
    ...
    <div v-if="showGambarPreviewModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 relative max-w-lg w-full flex flex-col items-center">
        ...
        <span class="text-gray-700">Preview Gambar Galery</span>
      </div>
    </div>
  </div>
</template>
...
<script setup>
import { useGaleryManagement } from "./useGaleryManagement.js";
